...
PRO_family_id = "00000000-0000-0000-0000-000000000002"
FREE_family_id = "00000000-0000-0000-0000-000000000001"
ENTERPRISE_family_id = "00000000-0000-0000-0000-000000000004"
...
async def test_get_latest_plan_returns_200(client: AsyncClient):
    response = await client.get(f"/api/v1/plans/{PRO_family_id}/versions/latest")
    assert response.status_code == 200
    body = response.json()
    assert body["success"] is True
...
async def test_get_latest_plan_returns_404_for_unknown_plan(client: AsyncClient):
    response = await client.get("/api/v1/plans/00000000-0000-0000-0000-999999999999/versions/latest")
...
async def test_get_plan_version_returns_200(client: AsyncClient):
    response = await client.get(f"/api/v1/plans/{PRO_family_id}/versions/1")
    assert response.status_code == 200
    body = response.json()
    assert body["data"]["id"] == PRO_family_id
    assert body["data"]["version"] == 1


async def test_get_plan_version_returns_404_for_nonexistent_version(client: AsyncClient):
    response = await client.get(f"/api/v1/plans/{PRO_family_id}/versions/99")
    assert response.status_code == 404
...
async def test_get_pricing_returns_monthly_and_annual_amounts(client: AsyncClient):
    response = await client.get(f"/api/v1/plans/{PRO_family_id}/versions/1/pricing?currency=NGN")
    assert response.status_code == 200
...
async def test_get_pricing_returns_custom_pricing_for_enterprise(client: AsyncClient):
    response = await client.get(f"/api/v1/plans/{ENTERPRISE_family_id}/versions/1/pricing?currency=NGN")
    assert response.status_code == 200
    data = response.json()["data"]
...
async def test_get_pricing_returns_404_for_unknown_currency(client: AsyncClient):
    response = await client.get(f"/api/v1/plans/{PRO_family_id}/versions/1/pricing?currency=JPY")
    assert response.status_code == 404
    assert response.json()["error"]["code"] == "pricing_not_found"


async def test_get_pricing_rejects_invalid_currency(client: AsyncClient):
    response = await client.get(f"/api/v1/plans/{PRO_family_id}/versions/1/pricing?currency=XX")
...
async def test_get_entitlements_returns_all_features(client: AsyncClient):
    response = await client.get(f"/api/v1/plans/{PRO_family_id}/versions/1/entitlements")
    assert response.status_code == 200
...
async def test_get_entitlement_by_key_returns_correct_value(client: AsyncClient):
    response = await client.get(f"/api/v1/plans/{PRO_family_id}/versions/1/entitlements/request_quota")
    assert response.status_code == 200
    data = response.json()["data"]
...
async def test_get_entitlement_by_key_returns_404_for_unknown_key(client: AsyncClient):
    response = await client.get(
        f"/api/v1/plans/{PRO_family_id}/versions/1/entitlements/nonexistent_feature"
    )
    assert response.status_code == 404
...
async def test_create_plan_returns_201(client: AsyncClient):
    response = await client.post("/api/v1/plans", json=VALID_PLAN_BODY, headers=ADMIN_HEADERS)
...
async def test_create_plan_version_increments_version(client: AsyncClient):
    ...
    create_response = await client.post("/api/v1/plans", json=VALID_PLAN_BODY, headers=ADMIN_HEADERS)
    ...
    response = await client.post(
        f"/api/v1/plans/{family_id}/versions",
        json=version_body,
...
async def test_create_plan_version_deactivates_previous_version(client: AsyncClient):
    ...
    v1_response = await client.get(f"/api/v1/plans/{family_id}/versions/1")
...
async def test_deactivate_plan_returns_200(client: AsyncClient):
    ...
    response = await client.patch(
        f"/api/v1/plans/{family_id}/versions/1/deactivate",
        headers=ADMIN_HEADERS,
    )
    assert response.status_code == 200
...
async def test_deactivate_plan_returns_409_if_already_inactive(client: AsyncClient):
    ...
    await client.patch(f"/api/v1/plans/{family_id}/versions/1/deactivate", headers=ADMIN_HEADERS)
    # Second deactivation
    response = await client.patch(
        f"/api/v1/plans/{family_id}/versions/1/deactivate", headers=ADMIN_HEADERS
    )
    assert response.status_code == 409
